 ...
 * 唤醒，合并所使用的资源列表
  ...
  例：ivw_wake_list = /abc/123/path1.irf;/abc/123/path2.irf;
     IVW_WAKE_LIST               @"ivw_wake_list"   //唤醒，合并的资源文件路径列表
...
/**
  创建唤醒实例，采用单例模式
 */
+ (id) createWakeuper:(NSString *)params delegate:(id<IFlyVoiceWakeuperDelegate>) delegate;

/**
  获取唤醒实例
 */
+(IFlyVoiceWakeuper *) getVoiceWakeuper;
 ...
 */
-(BOOL) writeAudio:(const void*)buffer  offset:(int)offset  length:(int)length;
