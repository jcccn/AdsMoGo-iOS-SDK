...
@interface YYPullUpRefreshView (Private)
- (void)setState:(YYPullRefreshState)aState;
@end

@implementation YYPullUpRefreshView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		[self setState:YYPullRefreshNormal];
...
- (void)setState:(YYPullRefreshState)aState{
	...
	switch (aState) {
		case YYPullRefreshPulling:
			...
			break;
		case YYPullRefreshNormal:
			
			if (_state == YYPullRefreshPulling) {
				[CATransaction begin];
				[CATransaction setAnimationDuration:FLIP_ANIMATION_DURATION];
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == YYPullRefreshLoading) {
        /*
		...
		if (_state == YYPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:YYPullRefreshNormal];
		} else if (_state == YYPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
			[self setState:YYPullRefreshPulling];
		}
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
		...
		[self setState:YYPullRefreshLoading];
