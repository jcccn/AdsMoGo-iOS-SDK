...
NSString *const YYSDWebImageDownloadStartNotification = @"YYSDWebImageDownloadStartNotification";
NSString *const YYSDWebImageDownloadStopNotification = @"YYSDWebImageDownloadStopNotification";

@interface YYSDWebImageDownloader ()
@property (nonatomic, retain) NSURLConnection *connection;
@end

@implementation YYSDWebImageDownloader
    ...
    if (NSClassFromString(@"SDNetworkActivityIndicator"))
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"startActivity")
                                                     name:YYSDWebImageDownloadStartNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"stopActivity")
                                                     name:YYSDWebImageDownloadStopNotification object:nil];
    }
    
    YYSDWebImageDownloader *downloader = [[[YYSDWebImageDownloader alloc] init] autorelease];
    downloader.url = url;
...
- (void)start
{
    ...
    if (connection)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YYSDWebImageDownloadStartNotification object:nil];
    }
    else
    {
...
- (void)cancel
{
    if (connection)
    {
        [connection cancel];
        self.connection = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:YYSDWebImageDownloadStopNotification object:nil];
    }
}
