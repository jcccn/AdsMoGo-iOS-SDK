...
static YYSDImageCache *instance;
...
+ (YYSDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[YYSDImageCache alloc] init];
    }
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <YYSDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <YYSDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
