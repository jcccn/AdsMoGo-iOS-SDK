...
-(void)share{
    ...
    if([self.infoData objectForKey:k_AdChinaSShareKitQzoneClientId]){
        tencentOAuth = [[TencentOAuth alloc] initWithAppId:[self.infoData objectForKey:k_AdChinaSShareKitQzoneClientId] andDelegate:self];
    }
...
-(void)resetOpenURLMethod{
    ...
    Method  old_method=class_getInstanceMethod(apClass, @selector(application:openURL:sourceApplication:annotation:));class_addMethod(apClass,NSSelectorFromString(@"mogoapplication:openURL:sourceApplication:annotation:"),method_getImplementation(old_method),method_getTypeEncoding(old_method));
...
-(BOOL)myapplication:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation{
    [QQApiInterface handleOpenURL:url delegate:[AdChinaSShareAdapterQzone shareMyClass]];
...
-(void)shareAudio{
    ...
    [QQApiAudioObject objectWithURL:[NSURL URLWithString:[self.infoData objectForKey:k_AdChinaSShareKitQQClickurl]]
                    ...
                    previewImageURL:[NSURL URLWithString:[self.infoData objectForKey:k_AdChinaSSahreKitQzoneThumbImage]]];
...
-(void)shareVideo{
    QQApiVideoObject *videoObj = [QQApiVideoObject objectWithURL:[NSURL URLWithString:[self.infoData objectForKey:k_AdChinaSShareKitQQClickurl]]
                                                           title:[self.infoData objectForKey:k_AdChinaSShareKitQzoneTitle]
                                                     description:[self.infoData objectForKey:k_AdChinaSSahreKitQzoneDetail]
                                                previewImageData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[self.infoData objectForKey:k_AdChinaSSahreKitQzoneThumbImage]]]];
...
- (void)onResp:(QQBaseResp *)resp{
    ...
    if (self.shareKit.delegate && [self.shareKit.delegate respondsToSelector:@selector(shareFinish:shareType:)]) {
        [self.shareKit.delegate performSelector:@selector(shareFinish:shareType:) withObject:[NSNumber numberWithBool:flag] withObject:[NSNumber numberWithInt:[AdChinaSShareAdapterQzone registryType]]];
    }
