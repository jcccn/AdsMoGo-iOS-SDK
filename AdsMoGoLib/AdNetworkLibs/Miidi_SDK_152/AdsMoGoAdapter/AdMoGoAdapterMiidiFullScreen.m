...
- (void)getAd {
	...
	if (type == AdViewTypeFullScreen || type==AdViewTypeiPadFullScreen) {
        ...
        if(![MiidiAdSpot isMiidiBasSpotAdReady]){ // 插屏广告还没有准备好
            ...
            [MiidiAdSpot requestMiidiBasSpotAd:^(NSError *error){ // 请求插屏数据源
                if(error != nil){ //失败
...
- (void)presentInterstitial{
    ...
    if (isReady) {
        ...
        // 已经准备好了
        if([MiidiAdSpot isMiidiBasSpotAdReady])
        {
            [MiidiAdSpot displayMiidiBasSpotAdWithBlock:[self getViewControllerToMiidi] withMiidiBasBlock:^(){ // 展示插屏广告
