...
- (void)getAd {
	...
	if (type == AdViewTypeFullScreen||
        ...
        if ([key isKindOfClass:[NSString class]]) {
            ...
            place = [key objectForKey:@"place"];
            if([place isEqualToString:@""] || place == NULL){
                place = @"default";
            }
        }
        mixview = [MIXView initWithID:adUnitId];
        [self adapterDidStartRequestAd:self];
        [MIXView preloadAdWithDelegate:self withPlace:place];
...
- (BOOL)isReadyPresentInterstitial{
    return [MIXView canServeAd:place];
}
...
//    [MIXView showAdWithDelegate:self];
    [MIXView showAdWithDelegate:self withPlace:place];
}
...
//预加载广告完成时调用
- (void)mixViewDidCacheAd:(MIXView *)view withPlace:(NSString *)place__{
    if([MIXView canServeAd:place__]){
