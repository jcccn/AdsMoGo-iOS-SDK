...
@interface AdMoGoAdapterAdwoFullAd (){
    
    UIView *adwoFullScreen;
}
...
//        [interstitial adapter:self didFailAd:nil];
    ...
    //这里建议用测试模式进去测试。
    adwoFullScreen = AdwoAdGetFullScreenAdHandle(pid,!testMode,self, adwo_ad_type);
    
    if(adwoFullScreen == nil)
    {
    ...
    //设置渠道号
    AdwoAdSetAGGChannel(adwoFullScreen, ADWOSDK_AGGREGATION_CHANNEL_MOGO);
    
    BOOL islocation = [configData islocationOn];
    if (islocation) {
        AdwoAdSetAdAttributes(adwoFullScreen, &(struct AdwoAdPreferenceSettings){
    ...
    if(!AdwoAdLoadFullScreenAd(adwoFullScreen, NO, NULL)){
        MGLog(MGT,@"fs load failed: %@", adwoResponseErrorInfoList[AdwoAdGetLatestErrorCode()]);
        [self adInterstitialFail];
        return;
    }
...
- (void)presentInterstitial{
    ...
    if(!AdwoAdShowFullScreenAd(adwoFullScreen)){
...
//    }else
//        MGLog(MGT,@"adwoAdViewDidLoadAd but interstitial is nil");
