...
- (void)getAd {
    ...
    [HMUserMessage shareInstance].hmChannel = @"IOS2.0";//渠道号，默认当前SDK版本号
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone){
        if (UIInterfaceOrientationIsLandscape(interfaceOrientation)) { //横屏
            ...
            [[HMInterstitial shareInstance]initWithFrame:rect andPicFrame:picRect
                                          andOrientation:@"Landscape"  andDelegate:self];
        }
        if (UIInterfaceOrientationIsPortrait(interfaceOrientation)) {//竖屏
            
            CGRect picRect = [self centerRectWithSize:CGSizeMake(270, 300)];
            [[HMInterstitial shareInstance]initWithFrame:rect andPicFrame:picRect
                                          andOrientation:@"Portrait" andDelegate:self];
        }
    }
    else
    {
